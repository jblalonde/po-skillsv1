...
**Canonical French headers** (use exactly these, local team convention):
- `### Critères d'acceptabilité` *(not « Critères d'acceptation »)*
- `### Comportement souhaité` *(adjustment-only)*
- `### Hors-portée` *(some teams use this in place of « Non-inclus », match the local style observed in Step 2)*
